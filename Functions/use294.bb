Function use294%()
    ...
    If (local5 <> 0) Then
        If (mousehit1 <> 0) Then
            ...
            If ((local5 And (input294 <> "")) <> 0) Then
                ...
                EndIf
                If (input294 <> "") Then
                    local10 = getinisectionlocation(scpmodding_processfilepath("Data\SCP-294.ini"), input294)
                EndIf
                If (local10 > $00) Then
                    local4 = getinistring2(scpmodding_processfilepath("Data\SCP-294.ini"), local10, "dispensesound", "")
                    If (local4 = "") Then
                        playerroom\Field9 = playsound_strict(loadtempsound(scpmodding_processfilepath("SFX\SCP\294\Dispense1.ogg")))
                    Else
                        playerroom\Field9 = playsound_strict(loadtempsound(local4))
                    EndIf
                    If (getiniint2(scpmodding_processfilepath("Data\SCP-294.ini"), local10, "explosion", "") = $01) Then
                        explosiontimer = 135.0
                        deathmsg = getinistring2(scpmodding_processfilepath("Data\SCP-294.ini"), local10, "deathmessage", "")
                    EndIf
                    local4 = getinistring2(scpmodding_processfilepath("Data\SCP-294.ini"), local10, "color", "")
                    ...
                    local16 = (Float getinistring2(scpmodding_processfilepath("Data\SCP-294.ini"), local10, "alpha", "1.0"))
                    local17 = getiniint2(scpmodding_processfilepath("Data\SCP-294.ini"), local10, "glow", "")
                    If (local17 <> 0) Then
                        local16 = (- local16)
                    EndIf
                    local18 = createitem("Cup", "cup", entityx(playerroom\Field19[$01], $01), entityy(playerroom\Field19[$01], $01), entityz(playerroom\Field19[$01], $01), local13, local14, local15, local16, $00)
                    local18\Field0 = ("Cup of " + input294)
                    entitytype(local18\Field1, $03, $00)
                Else
                    input294 = "OUT OF RANGE"
                    playerroom\Field9 = playsound_strict(loadtempsound(scpmodding_processfilepath("SFX\SCP\294\OutOfRange.ogg")))
                EndIf
            EndIf
        EndIf
    ...
    Else
        If (input294 <> scplang_getphrase("achievements.s10s")) Then
            input294 = scplang_getphrase("ingame.dispense")
        EndIf
