Function find860angle%(arg0.npcs, arg1.forest)
    ...
    Local local6%
    Local local7%
    tformpoint(entityx(collider, $00), entityy(collider, $00), entityz(collider, $00), $00, arg1\Field5)
    local0 = (Int floor(((tformedx() + 6.0) / 12.0)))
    local1 = (Int floor(((tformedz() + 6.0) / 12.0)))
    tformpoint(entityx(arg0\Field4, $00), entityy(arg0\Field4, $00), entityz(arg0\Field4, $00), $00, arg1\Field5)
    ...
    If (((local4 <> local0) Or (local5 <> local1)) <> 0) Then
        For local6 = (Int max((Float (local4 - $01)), 0.0)) To (Int min((Float (local4 + $01)), 9.0)) Step $01
            For local7 = (Int max((Float (local5 - $01)), 0.0)) To (Int min((Float (local5 + $01)), 9.0)) Step $01
                If ((((arg1\Field3[((local7 * $0A) + local6)] > $00) And ((local6 <> local4) Or (local7 <> local5))) And ((local6 = local4) Or (local7 = local5))) <> 0) Then
                    If (((Abs (local0 - local6)) + (Abs (local1 - local7))) < ((Abs (local0 - local4)) + (Abs (local1 - local5)))) Then
                        tformpoint(((Float local6) * 12.0), 0.0, ((Float local7) * 12.0), arg1\Field5, $00)
        ...
        Next
    Else
        Return (Int (point_direction(entityx(arg0\Field4, $00), entityz(arg0\Field4, $00), entityx(collider, $00), entityz(collider, $00)) + 180.0))
    EndIf
    Return $00
End Function
