Function loadmap%(arg0$)
            ...
            If (local14 <> Null) Then
                Select local3
                    Case "scp-860-1 endroom"
                        local14\Field3[((local2 * $0A) + local1)] = ($01 + local4)
                    Case "scp-860-1 path"
                        local14\Field3[((local2 * $0A) + local1)] = ($05 + local4)
                    Case "scp-860-1 corner"
                        local14\Field3[((local2 * $0A) + local1)] = ($09 + local4)
                    Case "scp-860-1 t-shaped path"
                        local14\Field3[((local2 * $0A) + local1)] = ($0D + local4)
                    Case "scp-860-1 4-way path"
                        local14\Field3[((local2 * $0A) + local1)] = ($11 + local4)
                    Case "scp-860-1 door"
                        local14\Field3[((local2 * $0A) + local1)] = ($15 + local4)
                End Select
            EndIf
        Next
        If (local14 <> Null) Then
            ...
            If (local16 <> Null) Then
                Select local3
                    Case "maintenance tunnel endroom"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $01
                    Case "maintenance tunnel corridor"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $02
                    Case "maintenance tunnel corner"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $03
                    Case "maintenance tunnel t-shaped room"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $04
                    Case "maintenance tunnel 4-way room"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $05
                    Case "maintenance tunnel elevator"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $06
                    Case "maintenance tunnel generator room"
                        local16\Field23\Field0[((local2 * $13) + local1)] = $07
                End Select
                local16\Field23\Field1[((local2 * $13) + local1)] = local4
            EndIf
        Next
    Else
        i_zone\Field0[$00] = $0D
    ...
    If (introenabled <> 0) Then
        local6 = createroom($00, $01, 8.0, 0.0, (Float ((mapheight + $02) Shl $03)), "room173intro")
    EndIf
    ...
    For local6 = Each rooms
        local6\Field24[$00] = Null
        local6\Field24[$01] = Null
        local6\Field24[$02] = Null
        local6\Field24[$03] = Null
        For local25 = Each rooms
            If (local6 <> local25) Then
                If (local6\Field5 = local25\Field5) Then
                    If ((local6\Field3 + 8.0) = local25\Field3) Then
                        local6\Field24[$00] = local25
                        If (local6\Field25[$00] = Null) Then
                            local6\Field25[$00] = local25\Field25[$02]
                        EndIf
                    ElseIf ((local6\Field3 - 8.0) = local25\Field3) Then
                        local6\Field24[$02] = local25
                        If (local6\Field25[$02] = Null) Then
                            local6\Field25[$02] = local25\Field25[$00]
                        EndIf
                    EndIf
                ElseIf (local6\Field3 = local25\Field3) Then
                    If ((local6\Field5 - 8.0) = local25\Field5) Then
                        local6\Field24[$01] = local25
                        If (local6\Field25[$01] = Null) Then
                            local6\Field25[$01] = local25\Field25[$03]
                        EndIf
                    ElseIf ((local6\Field5 + 8.0) = local25\Field5) Then
                        local6\Field24[$03] = local25
                        If (local6\Field25[$03] = Null) Then
                            local6\Field25[$03] = local25\Field25[$01]
                        EndIf
                    EndIf
                EndIf
            EndIf
            If (((((local6\Field24[$00] <> Null) And (local6\Field24[$01] <> Null)) And (local6\Field24[$02] <> Null)) And (local6\Field24[$03] <> Null)) <> 0) Then
                Exit
            EndIf
        Next
    Next
